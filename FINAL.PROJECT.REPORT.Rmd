---
title: "FINAL.PROJECT.REPORT.2025"
...
death_totals.2 <- data %>% 
  select(State, Deaths, Cause.Name,Year) %>%
  filter(State != 'United States', Cause.Name != 'All causes') %>%
  group_by(State,Year) %>%
  summarize(Total_Deaths = sum(Deaths, na.rm = TRUE))

TOTAL_US_DEATHS.2 <- data %>%
  select(State,Deaths,Year) %>%
  group_by(Year) %>%
  summarize(DEATHS = sum( Deaths))

COMBINE.2.DATA <- death_totals.2 %>% right_join(TOTAL_US_DEATHS.2, by = 'Year') %>%
  mutate(
    `Percentage of Deaths` = ((Total_Deaths/DEATHS)* 100)
  )

GRAPHED_DATA <- COMBINE.2.DATA %>%
  ggplot(aes(y = State, x = `Percentage of Deaths`, color = Year)) + geom_point()
